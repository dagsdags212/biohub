...
string comparison operation. A well-known approach in solving such problem us through the use of **dynamic
programming** where we attempt to break down the task into smaller subproblems and build an optimal solution
from smaller solutions. We'll illustrate dyanimc programming by exploring a few foundational algorithms.
...
gap penality uses different scores for gap openings and gap extensions. For the sake of simplicity, we will 
use a modified linear scheme based on the [EMBOSS Water](http://www.ebi.ac.uk/Tools/psa/emboss_water/) tool
where gaps are penalized by subtracting **10** from the score.
...
An extra row/column is needed to represent the starting point where $m=0$, $n=0$, and $s(S_0, T_0)=0$. After
creating matrix *M*, set the value of each element of the first row AND column to 0. The starting matrix is
illustrated below:
...
We label the **origin** as the element located at ($m=1$, $n=1$). 

When traversing the matrix, a downwards movement would indicate a gap introduced at string *S* while a 
rightward movement would introduce a gap at string *T*. Hence, movement from either direction would incur a 
...
$$
s_{\text{0,0}} = s(S_0, T_0) = 0
$$

$$
s_{1,1} = min(0, s_{0,0} -3) = 0
$$
